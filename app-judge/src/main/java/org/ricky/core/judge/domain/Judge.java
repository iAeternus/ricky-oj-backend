...
import static org.ricky.common.exception.ErrorCodeEnum.JUDGE_IS_OVER;
...
public class Judge extends AggregateRoot {
    ...
    public void checkJudgeNotOver() {
        if (nonNull(result)) {
            throw new MyException(JUDGE_IS_OVER, "Judge is over.", mapOf("judgeId", getId()));
        }
    }

    public void setJudgeResult(JudgeStatusEnum status, String errorMessage, Integer memory, Integer time, UserContext userContext) {
        this.status = status;
        this.result = JudgeResult.builder()
                .errorMessage(errorMessage)
                .memory(memory)
                .time(time)
                .build();
        addOpsLog("设置评测结果", userContext);
    }
