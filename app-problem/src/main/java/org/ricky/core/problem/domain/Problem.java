...
import org.ricky.core.problem.domain.event.*;
...
public class Problem extends AggregateRoot {
    ...
    public void updateSetting(ProblemSetting newSetting, String version, UserContext userContext) {
        ...
        checkCaseGroupsAndCasesAddition(oldContext, newContext, userContext);
    ...
    private void checkCaseGroupsAndCasesDeletion(ProblemSettingContext oldContext, ProblemSettingContext newContext, UserContext userContext) {
        ...
        if (isNotEmpty(deletedCaseGroupIds)) {
            raiseEvent(new CaseGroupDeletedEvent(getIdentifier(), deletedCaseGroups, userContext));
        }
        ...
        if (isNotEmpty(deletedCaseInfos)) {
            raiseEvent(new CaseDeletedEvent(getIdentifier(), deletedCaseInfos, userContext));
        }
    }

    private void checkCaseGroupsAndCasesAddition(ProblemSettingContext oldContext, ProblemSettingContext newContext, UserContext userContext) {
        Set<CaseGroupInfo> addedCaseGroups = oldContext.calculateAddedCaseGroups(newContext);
        Set<String> addedCaseGroupIds = addedCaseGroups.stream().map(CaseGroupInfo::getCaseGroupId).collect(toImmutableSet());
        if(isNotEmpty(addedCaseGroupIds)) {
            raiseEvent(new CaseGroupAddedEvent(getIdentifier(), addedCaseGroups, userContext));
        }

        Set<CaseInfo> addedCaseInfos = oldContext.calculateAddedCaseInfos(newContext, addedCaseGroupIds);
        if(isNotEmpty(addedCaseInfos)) {
            raiseEvent(new CaseAddedEvent(getIdentifier(), addedCaseInfos, userContext));
        }
    }
